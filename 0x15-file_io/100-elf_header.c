...
/**
 * print_magic -function prints the magic numbers
 * @e_ident: pointer to an array containing the ELF magic numbers
 ...
 * @e_type: pointer to the ELF type
 * @e_ident: pointer to an array containing the ELF class
 */
void print_type(unsigned int e_type, unsigned char *e_ident)
{
 ...
 * @e_entry: pointer address of the ELF entry point
 * @e_ident: pointer to an array containing the ELF class
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
 ...
 * @elf: pointer file descriptor of the ELF file
...
/**
 * main - function displays the information contained
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	i = read(fd, header, sizeof(Elf64_Ehdr));
	if (i == -1)
	{
		free(header);
		close_elf(fd);
